...
### /run の優先順位決定ロジック（常に「今最優先の1件」を特定して実行）
判定・着手は **この案件（`.env` の BACKLOG_PROJECT_ID）で絞った範囲のみ**。盤面は `scripts/next-target.mjs`（Ready）＋同じ案件名 relation フィルタで 進行中/レビュー待ち/未整理 を取得。
1. **中断中の 進行中 / レビュー待ち** があれば最優先で再開（既に「進行中」ならそのまま設計・実装を続ける）。
2. 次に **要件定義済み(Ready)** を優先順位順（緊急>高>中>低、同率は作成日古い順）で1件。**着手したらまず Notion ステータスを「進行中」に更新してから**設計・実装に入る。
3. Ready が空 → **未整理(Inbox)** を優先順位順で **1件ずつ** 判定：
   - **(a)** 意図・業務判断が必要（仕様が曖昧で人にしか決められない）／**本文が空・不足で要件化の材料が無い** → 人ボール「要回答」に記録（質問＋案）して **実装しない・LINE通知**、次の未整理へ。
   - **(b)** 情報十分で要件化できる（本文からストーリー＋AC＋スコープを起こせる） → CC が要件化 → ステータスを **要件定義済み** に更新 → **実装に入る時に「進行中」へ更新** → 実装まで自走（途中で意図必須と判明したら (a) に格下げ）。
   - 未整理を1件処理したら **1 に戻る**（Ready が生まれる／(a) で park／未整理が尽きる まで）。
4. **Ready も 未整理も無い**（中断中も無い） → 本当に手が無いので停止して報告（LINE通知）。
- **【重要】「未着手（未整理）だから検知できず止まる」状態をなくす。** Ready が無くても未整理から拾って前進（要件化 or 人ボール化）。停止するのは本当に手が無い時だけ。「Ready 空 → 必ず停止」は禁止。

### ステータス遷移（Notion 上で「今 AI が実装中のタスク」を可視化）
- 遷移は **要件定義済み →(着手)→ 進行中 →(実装・ローカル検証・dev マージ完了)→ 本番待ち**。
- **着手した瞬間に「進行中」に更新**することで、Notion 上で「今 AI が実装中のタスク」が一目で分かるようにする。原則 **同時に「進行中」になるのは1件**（1タスクずつ自走するため）。
- ステータス更新は Notion API（`notion-update-page` 相当）または既存スクリプトで行う。
- 実装・ローカル検証・dev マージが完了したら **「本番待ち」** に更新する（レビュー待ちは経由せず自己チェックで本番待ちへ）。
