...
- **タスクを手放す時はタグ「🤖AI実行中」を外す**：本番待ちに更新する時／人ボール化してスキップする時／中断する時。
...
- 実装・ローカル検証・dev マージが完了したら **「本番待ち」** に更新（＋タグ除去）。レビュー待ちは経由せず自己チェックで本番待ちへ。
...
**タイミング**：実装前でも、実装しながら／完了後でもよい（CC が適宜判断）。ただし **完了時には必ず Notion に存在し、ステータスが「完了」または「本番待ち」**になっている状態にする。
...
3. 実装 → ローカル検証 → dev マージ →「本番待ち」に更新（タグを外す）。
